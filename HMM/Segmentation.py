...
def train():
    # 初始化pi,A,B
    pi = [random.random() for _ in range(4)]    # 初始分布
    log_normalize(pi)
    A = [[random.random() for _ in range(4)] for _ in range(4)] # 转移矩阵：B/M/E/S
    A[0][0] = A[0][3] = A[1][0] = A[1][3]\
        = A[2][1] = A[2][2] = A[3][1] = A[3][2] = 0  # 不可能事件
    B = [[random.random() for _ in range(65536)] for _ in range(4)]
    for i in range(4):
        log_normalize(A[i])
        log_normalize(B[i])
    baum_welch(pi, A, B)
...
def load_train():
    f = open(".\\res\\pi.txt", mode="r")
    for line in f:
        pi = list(map(float, line.split(' ')[:-1]))
    f.close()

    f = open(".\\res\\A.txt", mode="r")
    A = [[] for _ in range(4)]  # 转移矩阵：B/M/E/S
    i = 0
    for line in f:
