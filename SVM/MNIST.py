...
FILE_PATH = './res/MNIST.train.csv'
FILE_TEST_PATH = './res/MNIST.test.csv'
...
if __name__ == "__main__":
    ...
    print('载入训练数据...')
    t = time()
    data = pd.read_csv(FILE_PATH, header=0, dtype=np.int)
    ...
    print('载入测试数据...')
    t = time()
    data_test = pd.read_csv(FILE_TEST_PATH, header=0, dtype=np.int)
