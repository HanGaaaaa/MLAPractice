...
def single_clf_set(classification):
    print(u'分类器：', classification)
    ...
    if hasattr(classification, 'C'):
        c_can = np.logspace(1, 3, 3)
        gamma_can = np.logspace(-3, 0, 3)
        model.set_params(param_grid={'C': c_can, 'gamma': gamma_can})
        m = c_can.size * gamma_can.size
    if hasattr(classification, 'max_depth'):
    ...
    end_time = time()
    t_train = (end_time - start_time) / (5*m)
    print(u'5折交叉验证的训练时间为：%.3f秒/(5*%d)=%.3f秒' % ((end_time - start_time), m, t_train))
    ...
    name = str(classification).split('(')[0]
