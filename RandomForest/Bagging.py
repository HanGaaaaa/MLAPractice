...
def f(input_x):
    return 0.5 * np.exp(-(input_x + 3) ** 2) + np.exp(-input_x ** 2) + 0.5 * np.exp(-(input_x - 3) ** 2)
...
if __name__ == "__main__":
    ...
    regs = [
        ...
        ('Ridge Regression(6 Degree)', ridged),
        ('Bagging Ridge(6 Degree)', bagging_ridged),
        ('Bagging DecisionTree Regression', BaggingRegressor(dtr, n_estimators=100, max_samples=0.3))]
    ...
    for i, (name, reg) in enumerate(regs):
        ...
        plt.plot(x_test, y_test.ravel(), color=colors[i], lw=i + 1, label=name, zorder=6 - i)
